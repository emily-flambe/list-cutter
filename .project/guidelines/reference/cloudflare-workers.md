...
> This guidance is adapted from https://developers.cloudflare.com/workers/prompt.txt
...
- Refer to the Code Examples section for Agents.
...
### Example: Durable Objects WebSocket
...
### Example: KV Session Authentication
...
### Example: Hyperdrive Connect to Postgres
...
**Description:** Using Workflows for durable execution, async tasks, and human-in-the-loop workflows.
...
### Example: Workers Analytics Engine

**Description:** Using Workers Analytics Engine for writing event data.
...
### Example: Browser Rendering Workers

**Description:** Use the Browser Rendering API as a headless browser to interact with websites from a Cloudflare Worker.
...
**Description:** Serve Static Assets from a Cloudflare Worker and/or configure a Single Page Application (SPA) to correctly handle HTTP 404 (Not Found) requests and route them to the entrypoint.
...
**Description:** Build an AI Agent on Cloudflare Workers, using the agents, and the state management and syncing APIs built into the agents.
...
### Example: Workers AI Structured Outputs JSON

**Description:** Workers AI supports structured JSON outputs with JSON mode, which supports the `response_format` API provided by the OpenAI SDK.
...
### Pattern: WebSocket Coordination

**Description:** Fan-in/fan-out for WebSockets. Uses the Hibernatable WebSockets API within Durable Objects. Does NOT use the legacy addEventListener API.
